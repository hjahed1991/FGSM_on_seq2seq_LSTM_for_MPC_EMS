...
# --- Determine Clean Run Directory and Import Model Utils --- (NEW)
...
MODEL_LOAD_PATH = CLEAN_ARTIFACTS_SAVE_PATH / 'models' # Match train_model.py format
ARTIFACTS_LOAD_PATH = CLEAN_ARTIFACTS_SAVE_PATH / 'artifacts'
...
scaler_path = UPPER_ARTIFACTS_LOAD_PATH / "target_scaler.joblib"
try:
...
plt.plot(test_timestamps, predictions_actual_scale[:, prediction_step_to_plot], label=f'Predicted ({MODEL_TYPE}, {PROCESS_TYPE})', color='red', marker='x', markersize=4, linestyle='--')

plt.xlabel("Time")
plt.ylabel(TARGET_COLUMN.replace('_', ' ').title())
title_step1 = f"{TARGET_COLUMN} - Test Set ({PROCESS_TYPE}-{MODEL_TYPE}-horizon_{FORECAST_HORIZON}-seed_{RANDOM_SEED}): Actual vs Predicted ({prediction_step_to_plot+1}-step ahead)\n"
...
try:
    pred_df = pd.DataFrame({
        ...
        f'Predicted_{TARGET_COLUMN}_{PROCESS_TYPE}_{MODEL_TYPE}_horizon_{FORECAST_HORIZON}_seed_{RANDOM_SEED}': predictions_actual_scale[:, prediction_step_to_plot]
    })
...
if forecast_start_index >= len(test_timestamps):
    ...
    plt.plot(forecast_timestamps_window,
             ...
             label=f'Forecast ({MODEL_TYPE}, {PROCESS_TYPE})', color='red', marker='x', linestyle='--')

    plt.xlabel("Time")
    plt.ylabel(TARGET_COLUMN.replace('_', ' ').title())
    plt.title(f"Multi-Step Forecast vs Actual ({TARGET_COLUMN} - {PROCESS_TYPE} - {MODEL_TYPE} - horizon_{FORECAST_HORIZON} - seed_{RANDOM_SEED}) - Starting index {forecast_start_index}")
    plt.legend()
    plt.grid(True)
    plt.xticks(rotation=45)
    plt.tight_layout()
    # Make filename dynamic
    plot_save_path_window = PLOTS_SAVE_PATH / "Multi-Step Forecast vs Actual.png"
