...
# DATA_PATH = rf'D:\cursor\DP_SIEMS\data\Rye_microgrid\new_dataset.csv' # Keep data path fixed for now, or adjust based on TARGET_COLUMN if needed
DATA_PATH = POISONED_LOAD_PATH / f"poisoned_datasets_{ATTACK_TYPE}"


# --- Load Poisoned Dataset (Dynamic) ---
print(f"--- Loading {ATTACK_TYPE}-{PROCESS_TYPE}-epsilon_{EPSILON}-ratio_{POISONING_RATIO} Dataset from ({DATA_PATH}) ---")
try:
    datasets = list(DATA_PATH.glob("*.csv"))
    if not datasets:
        raise FileNotFoundError(f"No poisoned dataset found in {DATA_PATH}")
    last_poisoned_dataset_path = max(datasets, key=os.path.getctime)
    print(f"Found last poisoned dataset: {last_poisoned_dataset_path}")
except FileNotFoundError as e:
    print(f"Error: {e}. Run generate_adversarial_samples.py for making poisoned datasets first.")
    exit()
except Exception as e:
    print(f"Error loading {ATTACK_TYPE} poisoned dataset from {last_poisoned_dataset_path}: {e}")
    exit()
DATA_PATH = last_poisoned_dataset_path
...
ARTIFACTS_LOAD_PATH = CLEAN_ARTIFACTS_LOAD_PATH / 'artifacts' # Load CLEAN artifacts from this path

MODEL_SAVE_PATH = POISONED_LOAD_PATH / 'models'
ARTIFACTS_SAVE_PATH = POISONED_LOAD_PATH / 'artifacts' # Save poisoned artifacts to this path
...
# --- Load Hyperparameters from Fine-tuned Run ---
print("\n--- Loading Hyperparameters from Fine-tuned Run ---")

# Load hyperparameters
hyperparams_path = UPPER_ARTIFACTS_LOAD_PATH / "best_hyperparams.json"
try:
...
#  --- Load and Preprocess Poisoned Dataset  ---
print("\n--- Loading and Preprocessing Data ---")

df = pd.read_csv(DATA_PATH,
                 parse_dates=[TIMESTAMP_COLUMN],
                 index_col=[TIMESTAMP_COLUMN]) # Load with parsing/indexing
...
# Scaling data (fit on train, transform train, val, and test)
print("\n--- Scaling Data ---")
...
print(train_df_scaled.agg(['min', 'max']))
...
class LossHistory(Callback):
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        elif hasattr(pl_module, 'current_train_loss') and pl_module.current_train_loss is not None: # Fallback
...
try:
    if MODEL_TYPE == 'LSTM':
        ...
        model_init_params['hidden_size'] = best_hyperparams['hidden_size']
        model_init_params['num_layers'] = best_hyperparams['num_layers']
        model_init_params['teacher_forcing_ratio'] = best_hyperparams.get('teacher_forcing_ratio', 0.5) # Added for GRUForecastModel2
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename=f'{MODEL_TYPE}-best-model-{PROCESS_TYPE}-{ATTACK_TYPE}-epsilon_{EPSILON}-ratio_{POISONING_RATIO}-{{epoch:02d}}-{{val_loss:.4f}}', # Filename reflects val_loss
    save_top_k=5, mode='min', verbose=True # Save top 5
)
...
final_logger = TensorBoardLogger(save_dir=POISONED_LOAD_PATH, name="lightning_logs") # Saves logs relative to script
...
print(f"\nFinal training finished. Best {MODEL_TYPE}-{PROCESS_TYPE}-{ATTACK_TYPE}-epsilon_{EPSILON}-ratio_{POISONING_RATIO} model saved at: {best_model_path}")
...
elif val_loss_history and not all(np.isnan(v) for v in val_loss_history): # Fallback to min val loss
...
plt.xlabel("Epoch")
plt.ylabel("Loss (MSE)")
plt.title(f"Training & Validation Loss Curve for {TARGET_COLUMN}-{MODEL_TYPE}-horizon={FORECAST_HORIZON}\n{ATTACK_TYPE}-{PROCESS_TYPE}-epsilon={EPSILON}-Poisoning_ratio={POISONING_RATIO}") # Updated title
plt.legend()
plt.grid(True)
...
loss_data_save_dir = EXCEL_SAVE_PATH # Save alongside other artifacts
